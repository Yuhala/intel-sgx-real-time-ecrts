...
#include "restrain_posix_calls.h"
...
#endif
#include <limits.h>
#ifndef USE_PTHREAD_SELF_INSTEAD_OF_GETTID_SYSCALL
#include <linux/unistd.h>
#endif

#ifndef DISABLE_PRCTL
#include <sys/prctl.h>
#endif
#include <sys/stat.h>
...
#ifndef DISABLE_RAISE_SOFT_PRIO
/*
...
#else
			err_msg("sched_setscheduler failed and soft prio disabled.\n");
			exit(-1);
#endif
#ifndef DISABLE_ERRNO
		}
...
void *timerthread(void *param)
{
	...
	if (par->cpu != -1) {
...
#ifndef USE_SCHED_INSTEAD_OF_PTHREAD_AFFINITY
		thread = pthread_self();
		...
		if(sched_setaffinity(0, sizeof(mask), &mask) == -1) {
			err_msg("Could not set CPU affinity to CPU #%d\n", par->cpu);
			exit(-1);
		}
#endif
	}
...
#else
		printf("Cyclic timer mode is not supported,\n");
			return NULL;
#endif
	}
...
#else
		printf("Itimer mode is not supported.\n");
		return NULL;
#endif
	}
...
#ifndef SET_MAX_CPU_CORES
	int max_cpus = sysconf(_SC_NPROCESSORS_CONF);
#else
	int max_cpus = SET_MAX_CPU_CORES;
#endif
...
static int check_kernel(void)
{
	...
	if (ret) {
		...
		fprintf(stderr, "uname failed: %s. Assuming not 2.6\n",
				strerror(errno));
#else
		fprintf(stderr, "uname failed. Assuming not 2.6\n");
#endif
...
#ifndef DISABLE_PTHREAD_KILL
			pthread_kill(statistics[i]->thread, SIGTERM);
#else
			warn("pthread_kill is not supported.\n");
#endif
		}
